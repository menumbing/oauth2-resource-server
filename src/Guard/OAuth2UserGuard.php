 ...
 */
class OAuth2UserGuard extends AbstractOAuth2Guard
{
    protected function authenticateWithBearerToken(ServerRequestInterface $request): ?AuthenticatableInterface
    {
        ...
        if (null === $user = $this->provider->retrieveByToken($request->getAttribute('oauth_user_id'), $bearerToken)) {
            return null;
        }

        if (null === $this->retrieveClient($request->getAttribute('oauth_client_id'), $bearerToken)) {
            return null;
        }
